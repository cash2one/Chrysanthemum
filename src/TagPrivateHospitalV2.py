...
def findAndInsert(conn, cpPropCursor, tagDefCursor, cpTagCursor, cpTagResCursor, brandCursor, tagName, point_name, prop_value = ''):
    ...
    if len(prop_value) > 0:
        cpPropSql = "select cp.point_code, cp.ref_area_code, p.point_name from tbl_cp cp inner join tbl_cp_prop p on cp.point_code = p.ref_cp_code left join tbl_cp_exprop ex on cp.point_code = ex.ref_cp_code where p.ref_cptype_code like 'CP-PRIVATE-HOSPITAL%' and p.point_name like  '%" + point_name + "%' or ex.prop_value like '%" + prop_value + "%'"
        ...
        cpPropSql = "select cp.point_code, cp.ref_area_code, p.point_name from tbl_cp cp inner join tbl_cp_prop p on cp.point_code = p.ref_cp_code left join tbl_cp_exprop ex on cp.point_code = ex.ref_cp_code where p.ref_cptype_code like 'CP-PRIVATE-HOSPITAL%' and p.point_name like  '%" + point_name + "%'"
        cpPropCursor.execute(cpPropSql)
