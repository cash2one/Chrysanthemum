...
def main():
	...
	for tagRow in tagCursor:
		...
		" '%" + tagRow['name'] + "%' AND prop.ref_cptype_code LIKE 'CP-HOUSESELL%'"
		...
		for cpPropRow in cpPropCursor:
			tmpDict = {
				...
				'ref_brand_code': getBrandCode(conn, brandCursor, cpPropRow['point_name'], 'HOUSESELL')
			}
		...
		cpTagResDict = []
		for cpPropRow in cpPropCursor:
			if isCPType(conn, cpProp1Cursor, cpPropRow['ref_cp_code'], 'HOUSESELL'):
				tmpDict = {
					'ref_cp_code': cpPropRow['ref_cp_code'],
					'ref_tag_definition_id': tagRow['id'],
					'ref_area_code': getRefAreaCode(conn, cpCursor, cpPropRow['ref_cp_code']),
					'ref_brand_code': getBrandCode(conn, brandCursor, cpPropRow['prop_value'], 'HOUSESELL')
				}
